...
# 遍历每个特征进行单独预测
def run_feature_prediction(feature_idx, feature_name, model_type='LSTM'):
    print(f"\n=== 预测特征：{feature_name}，模型：{model_type} ===")
    ...
    model = LSTM(input_size=8, hidden_size=64, output_size=1).to(device) if model_type == 'LSTM' else LNN(input_size=8, hidden_size=64, output_size=1).to(device)
    train_model(model, train_loader, epochs=50)
    evaluate_model(model, test_loader, feature_name)

# 针对每个特征进行独立预测（LSTM和LNN对比）
for idx, feature in enumerate(features):
    run_feature_prediction(idx, feature, model_type='LSTM')
    run_feature_prediction(idx, feature, model_type='LNN')
